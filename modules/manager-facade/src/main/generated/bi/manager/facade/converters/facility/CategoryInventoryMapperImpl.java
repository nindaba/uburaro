...
public class CategoryInventoryMapperImpl implements CategoryInventoryMapper {
    ...
    public MBCategoryType categoryToType(MBCategoryData category) {
        ...
        mBCategoryType.setCode( category.getCode() );
        mBCategoryType.setVisible( category.isVisible() );
        mBCategoryType.setActive( category.isActive() );
        mBCategoryType.setModificationLogs( modificationLogDataListToModificationLogTypeSet( category.getModificationLogs() ) );
        mBCategoryType.setName( category.getName() );
        mBCategoryType.setInventories( mBInventoryDataSetToMBInventoryTypeSet( category.getInventories() ) );
        mBCategoryType.setFacility( mBFacilityDataToMBFacilityType( category.getFacility() ) );
    ...
    protected Set<MBInventoryData> mBInventoryTypeSetToMBInventoryDataSet(Set<MBInventoryType> set) {
        ...
        for ( MBInventoryType mBInventoryType : set ) {
            set1.add( inventoryToData( mBInventoryType ) );
        }
    ...
    protected MBInventoryType mBInventoryDataToMBInventoryType(MBInventoryData mBInventoryData) {
        ...
        mBInventoryType.setCategory( categoryToType( mBInventoryData.getCategory() ) );
    ...
    protected Set<MBCategoryType> mBCategoryDataSetToMBCategoryTypeSet(Set<MBCategoryData> set) {
        ...
        for ( MBCategoryData mBCategoryData : set ) {
            set1.add( categoryToType( mBCategoryData ) );
        }
    ...
    protected MBClientType mBClientDataToMBClientType(MBClientData mBClientData) {
        if ( mBClientData == null ) {
            return null;
        }

        MBClientType mBClientType = new MBClientType();

        mBClientType.setCode( mBClientData.getCode() );
        mBClientType.setVisible( mBClientData.isVisible() );
        mBClientType.setActive( mBClientData.isActive() );
        mBClientType.setModificationLogs( modificationLogDataListToModificationLogTypeSet( mBClientData.getModificationLogs() ) );
        mBClientType.setName( mBClientData.getName() );
        mBClientType.setAddress( mBClientData.getAddress() );
        mBClientType.setTotalDebt( mBClientData.getTotalDebt() );
        mBClientType.setOrders( mBOrderDataSetToMBOrderTypeSet( mBClientData.getOrders() ) );
        mBClientType.setInvoices( mBInvoiceDataSetToMBInvoiceTypeSet( mBClientData.getInvoices() ) );
        mBClientType.setFacility( mBFacilityDataToMBFacilityType( mBClientData.getFacility() ) );
        mBClientType.setContracts( mBRentContractDataSetToMBRentContractTypeSet( mBClientData.getContracts() ) );
    ...
    protected Set<MBClientType> mBClientDataSetToMBClientTypeSet(Set<MBClientData> set) {
        ...
        for ( MBClientData mBClientData : set ) {
            set1.add( mBClientDataToMBClientType( mBClientData ) );
        }
